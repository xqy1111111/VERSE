    ...
    def forward(self, input_tensor, attention_mask, return_attention=False):
        ...
        if return_attention:
            self_output, attention_probs = self.self(
                input_tensor, input_tensor, input_tensor, attention_mask, return_attention=True
            )
        else:
...
class CrossAttentionLayer(nn.Module):
    def __init__(self, config):
        ...
        self.cross_att = BertSelfAttention(config)
        self.cross_output = BertSelfOutput(config)
        self.intermediate = BertIntermediate(config)
        self.output = BertOutput(config)

    def forward(self, query_states, key_value_states, attention_mask, return_attention=False):
        """
        Args:
            query_states: (N, Lq, D)
            key_value_states: (N, Lk, D)
            attention_mask: (N, Lq, Lk)
        """
        if return_attention:
            cross_context, attention_probs = self.cross_att(
                query_states, key_value_states, key_value_states, attention_mask, return_attention=True
            )
        else:
            cross_context = self.cross_att(query_states, key_value_states, key_value_states, attention_mask)
            attention_probs = None
        attn_output = self.cross_output(cross_context, query_states)
